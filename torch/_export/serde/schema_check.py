    ...
    def _handle_aggregate(ty) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        for f in dataclasses.fields(ty):
            ...
            if isinstance(f.type, str):
                # NOTE: If we have a field with string-annotated type, we should treat it
                #       as a back reference in circular definition and generate pointer type.
                cpp_ret[f.name]["to_prefix"] = "*"
                cpp_ret[f.name]["from_prefix"] = f"std::make_unique<{f.type}>("
                cpp_ret[f.name]["from_suffix"] = ")"
                cpp_ret[f.name]["move_only"] = True
...
"""
        ...
        to_json_def = f"""{{
{chr(10).join([f'  nlohmann_json_j["{name}"] = {f.get("to_prefix", "")}nlohmann_json_t.{name};' for name, f in cpp_fields.items()])}
}}
"""
        from_json_decl = f"void from_json(const nlohmann::json& nlohmann_json_j, {name}& nlohmann_json_t)"

        def from_json_call(name, f):
            return (
                f.get("from_prefix", "")
                + f'nlohmann_json_j.value("{name}", {f.get("to_prefix", "")}nlohmann_json_default_obj.{name})'
            )

        from_json_def = f"""{{
  {name} nlohmann_json_default_obj;
{chr(10).join(
    [f'  nlohmann_json_t.{name} = {from_json_call(name, f)}{f.get("from_suffix", "")};'
    for name, f in cpp_fields.items()])}
}}
"""
        default_ctor_def = " = default;"
        assignment_operator_def = " = default;"
        if any(f.get("move_only", False) for f in cpp_fields.values()):
            default_ctor_def = f""" {{
{chr(10).join([
    f'    {name} = {f.get("from_prefix", "")}{f.get("from_suffix", "")};'
    for name, f in cpp_fields.items() if f.get("move_only", False)])}
  }}"""
            assignment_operator_def = f""" {{
{chr(10).join([
    f'    {name} = {f.get("from_prefix", "")}{f.get("to_prefix", "")}other.{name}{f.get("from_suffix", "")};'
    for name, f in cpp_fields.items()])}
    return *this;
  }}"""
        cpp_class_defs[
...
{field_decls}
 ...
 public:
  {name}(){default_ctor_def}
  {name}({name}&&) = default;
  {name}& operator=({name}&&) = default;
  {name}(const {name}& other): {", ".join([
    f'{name}({f.get("from_prefix", "")}{f.get("to_prefix", "")}other.{name}{f.get("from_suffix", "")})'
    for name, f in cpp_fields.items()])} {{}}
  {name}& operator=(const {name}& other){assignment_operator_def}
